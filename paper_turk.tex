...
\item {\bf Duration of the experiment:} The duration of each experiment is a period of time set so that the task is not available after that time period no matter whether all the required number of workers have participated or not. 

\item {\bf Redirect to new web:} The MTurk worker should be informed clearly that they will be redirect to a new website from where they will provide feedback. The whole task including the instruction and procedure in the web application presented as per the task plan described in Section \ref{sec:task_plan}.
